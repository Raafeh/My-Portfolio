...
const Nav = () => {
  ...
  mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen'>
    ...
    xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] xl:h-max py-8 bg-white/10 blackdrop-blur-sm text-3xl xl:text-xl xl:rounded-full '>
      {navData.map((link, index) => {
        return( 
        <Link className={`${link.path === pathname && 'text-accent'} relative flex items-center group transition-all duration-300 hover:text-accent`} 
          href={link.path} 
          key={index}>
          {/* tooltip */}
